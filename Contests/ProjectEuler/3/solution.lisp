...
(defun a-generator ()
  (declare (special sieve-size n log10n m fb fb-size ))
  (let* (
...
;; (defparameter *qs-line-width* 74)
;; (defparameter *qs-line* "--------------------------------------------------------------------------")
...
(defun report?sieving-time ()
  (declare (special  time1))
  (when report (report))
  (- (minutes) time1))

(defun report?total-time ()
  (declare (special  time0 factors-of-n slp-hash slp-table
...
(defun qs (n &key (m 0)                 ; [-M...+M] is the sieve
  ...
  ;; (declare (special n m fb-start  delta-t return))
  (centered-report n)
